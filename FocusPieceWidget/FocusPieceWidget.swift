...
/// "✓ 3 | ▢ 8 | 1 Std 15 Min" — revealed works, collection size, focus time.
...
/// Medium widget — label + timer + stats on the left, the Start pill right.
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 4) {
                Image(systemName: "checkmark.seal.fill")
                    ...
                    .foregroundStyle(Theme.Palette.accent)
                Text("\(snapshot.worksUnlocked) · \(snapshot.focusTimeString)")
...
private struct AccessoryFocusView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Text("\(snapshot.worksUnlocked) enthüllt · \(snapshot.focusTimeString)")
                .font(Theme.Font.sans(11, weight: .medium))
                .opacity(0.8)
        }
